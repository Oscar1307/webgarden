...
inner join marcaruta as ma  on ru.idmarca=ma.idmarca;";
    ...
    function selalum($idacudiente) {
     $sql = "select alu.identificacion,alu.foto,alu.direccion ,alu.prinombre,alu.priapellido,cur.nombre curso ,val1.nombre as rh ,val2.nombre as grupos from alumno as alu inner join curso as cur on alu.idcurso=cur.idcurso inner join valor as val1 on alu.rh=val1.idvalor inner join valor as val2 on alu.gruposan=val2.idvalor where acudiente='" .$idacudiente. "'; ";
        $conexionBD = new conexion();
        $conexionBD->conectarBD();
        $data = $conexionBD->ejeCon($sql, 0);
        return $data;
